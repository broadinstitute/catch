...
A dataset with 1 'Human picobirnavirus' sequences. The virus is
...
chrs = ["segment_" + seg for seg in ['2']]

def seq_header_to_chr(header):
    import re
    c = re.compile(r'\[segment (2)\]')
    m = c.search(header)
    if not m:
...
for f in listdir(join(dirname(__file__), "data/human_picobirnavirus/")):
    ds.add_fasta_path("data/human_picobirnavirus/" + f, relative=True)
